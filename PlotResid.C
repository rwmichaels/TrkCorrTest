...
void plot_resid() {
   ...
   HrsTrkCorr *trkcorr = new HrsTrkCorr(HrsTrkCorr::kRight);  //kLeft or kRight
   ...
   for (Int_t ixbeam=0; ixbeam<MAXXBEAM; ixbeam++) { // X=0, X=-3, X=+3
	...
	for (Int_t irow=0; irow<MAXROW; irow++) {
	  ...
	  // convert rad back to mrad
          tgph = 1000.*trkcorr->GetResidTgPh(ixbeam,icol,irow);
          tgth = 1000.*trkcorr->GetResidTgTh(ixbeam,icol,irow);
// for defined residuals
          if (tgph > -200 && tgth > -200) { 
  	      xx[npt]  = 1.0*idx;
	      yy1[npt] = tgth;
	      yy2[npt] = tgph;
	      ex[npt]  = 0.00001;
   ...
   leg1->AddEntry(grth[0]," X = 0 ","p");
   leg1->AddEntry(grth[1]," X = -3.2 mm","p");
   leg1->AddEntry(grth[2]," X = +3.8 mm","p");
   leg1->Draw();

   c1->cd(2);
   TLegend *leg2 = new TLegend(0.55,0.7,0.89,0.89);
   leg2->AddEntry(grph[0]," X = 0 ","p");
   leg2->AddEntry(grph[1]," X = -3.2 mm","p");
